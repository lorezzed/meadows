...
export function simulate(system: System): StockSeries[] {
  ...
  // Each faucet moves rate(t)·DT per step from its source stock (the first
  ...
  // the first on either side are ignored (v1 rule). A faucet's rate is its
  // schedule's last segment whose start time is ≤ t: the initial `value` at
  // t=0 plus any `steps`, kept sorted by time (stable, so a step written at
  // t=0 overrides the initial).
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const rates = faucets.map(f => rateAt(f.segs, i * DT));
